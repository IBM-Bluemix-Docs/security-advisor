...
| Finding           | Severity and Risk              | Remediation           |
|:------------------|:------------------|:----------------------|
| `COS bucket is not encrypted with a user-managed key` | High severity: A storage bucket is not encrypted by a key that is managed with Key Protect or Hyper Protect Crypto Services. | To fix the issue, use a key management service, such as {{site.data.keyword.keymanagementservicelong_notm}} or Hyper Protect Crypto Services, to control the encryption key for the bucket. |
