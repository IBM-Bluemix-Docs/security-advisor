...
IBM X-Force によるクライアントの動作パターンの分類で、スキャナーとして使用される、ボットネットの一部として使用される、暗号通貨のマイニングに使用される、あるいは匿名化サービスに使用されるマルウェアを配布しているとされたパターンは、すべて継続的にモニターされます。 そうしたクライアントがモニター対象のクラスターに接近し、警戒すべき動作が見られた場合、Network Insights は検出事項を送出します。
